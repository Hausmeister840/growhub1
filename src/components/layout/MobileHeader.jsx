...
export default function MobileHeader({ user, currentPageName, onMenuClick, hidden }) {
  ...
  return (
    <>
      ...
      >
        <div className="absolute inset-0 gh-glass border-b border-white/[0.1]" />
        <div className="pointer-events-none absolute inset-x-0 bottom-0 h-6 bg-gradient-to-b from-transparent to-black/20" />
        
        <div className="relative flex items-center justify-between h-14 px-4 py-2.5">
          {/* Left: Logo or Title */}
          {showLogo ? (
          ...
          <div className="flex items-center gap-1">
            <Link
              to="/Search"
            className="w-9 h-9 flex items-center justify-center rounded-full text-[var(--gh-text-secondary)] hover:text-white hover:bg-white/[0.1] transition-all"
              aria-label="Suche"
            >
            ...
            {user && (
              <>
                <Link
                  to="/Messages"
                  className="relative w-9 h-9 flex items-center justify-center rounded-full text-[var(--gh-text-secondary)] hover:text-white hover:bg-white/[0.1] transition-all"
            ...
            <button
              onClick={onMenuClick}
              className="ml-0.5 w-9 h-9 flex items-center justify-center rounded-full hover:bg-white/[0.08] transition-colors"
              aria-label="Menü"
            >
              {user?.avatar_url ? (
                <img 
